<x-app-layout>
    ...
    <div class="py-12">
        @if(session()->has('message'))
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative max-w-xl mx-auto mb-4" role="alert">
                        ...
                        @foreach($bidDetails as $bidDetail)
                            <tr class="text-white">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('bid.delete', ['bid' => $bidDetail['bid_id']]) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <a href="{{ route('advertisements.show', ['advertisement' => $bidDetail['advertisement_id']]) }}" class="inline-flex items-center rounded-md bg-blue-400 px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 ml-2">
                                        {{ __('Details') }}
                                    </a>
                                </td>
                            </tr>
